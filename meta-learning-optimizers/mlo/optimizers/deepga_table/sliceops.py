...
def multi_slice_add_with_multiplier(x1_inplace, x2, x1_slices=(), x2_slices=(), multiplier=1.0):
    """
    ...
    if (len(x1_slices) != 0) and (len(x2_slices) == 0):
        for x1_slice in x1_slices:
            x1_inplace[x1_slice] += x2 * multiplier

    elif (len(x1_slices) != 0) and (len(x2_slices) != 0) \
            and (len(x2_slices) == len(x1_slices)):

        for i in range(len(x1_slices)):
            x1_inplace[x1_slices[i]] += x2[x2_slices[i]] * multiplier

    elif (len(x1_slices) == 0) and (len(x2_slices) == 0):
        x1_inplace += x2 * multiplier
