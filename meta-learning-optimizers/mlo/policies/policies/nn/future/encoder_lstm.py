...
class EncoderLSTM(Policy):
    r"""Class for Deep Neural Network as Policy Model"""

    def __init__(self, config):
        print("EncoderLSTM not implemented yet!")
        exit()
        ...
        # Create LSTM_2:
        _in_size = self.enc_layers[-1] + self.lstm_1_hidden_size + (self.repeat_output * self.enc_dim)
        self.lstm_2 = nn.LSTM(_in_size, self.lstm_2_hidden_size, self.lstm_2_num_layers)

        # Decoder:
        self.dec = nn.Sequential(BayesLinear(self.lstm_2_hidden_size, self.enc_dim), nn.Sigmoid())
    ...
    def forward(self, observation, reward, time, action):
        ...
        if observation is None and action is None:
            ...
            # Action:
            action = np.ones((self.repeat_output, self.enc_dim)) * (-1.0)
        ...
        with torch.no_grad():
            ...
            # Sampler:
            prediction = torch.Tensor(self.repeat_output, self.enc_dim)
